// const banList = ["681701089"] // TODO DB
...
async function reactToNewMessage(node: Element): Promise<void> {
   ...
   * if the "data-peer" attribute is found in the banList
   * @type {Element | null}
   */
  ...
  const storage = await chrome.storage.local.get(["banList"]);
  if (storage.banList && storage.banList.includes(messagePeerID)) {
    messageStack.remove();
  } else if (
    messagePeerID &&
    messageActionsBox &&
    messageActionsBox.childElementCount < 5 // action-box was not modified 
  ) {
    let hideButton: HTMLSpanElement = Object.assign(
      document.createElement("span"),
      {
        role: "link",
        ariaLabel: "Hide",
        className: "im-mess--reply _im_mess_hide", // FIXME
        innerHTML: `
			<svg
				width="16"
				height="16"
    ...
    hideButton.addEventListener("click", async (event: MouseEvent) => {
      chrome.storage.local.get(["banList"], async (data) => {
        ...
        if (Object.keys(data).length == 0) {
          chrome.storage.local.set({ banList: [messagePeerID] });
        } else {
          chrome.storage.local.set({
            banList: [...data.banList, messagePeerID],
          });
        }
      });
    });

    messageActionsBox.insertAdjacentElement("beforeend", hideButton);
  }
}
